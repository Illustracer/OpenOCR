...
def print_model_structure(model: nn.Module, max_depth: int = 5):
    """打印模型结构"""
    for name, module in model.named_modules():
        depth = name.count('.')
        if depth < max_depth:
            indent = "  " * depth
            qconfig_status = "✓ 量化" if hasattr(module, 'qconfig') and module.qconfig else "✗ 不量化"
...
class QuantTrainer(object):
    ...
    def __init__(self, cfg, mode='train', task='rec'):
        ...
        # 进行量化前处理
        rep_layers = find_layers_by_name(self.model, "QuantizableLearnableRepLayer")
        ...
        # amp
        self.scaler = (torch.amp.GradScaler('cuda') if self.cfg['Global'].get(
